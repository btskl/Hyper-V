. 'Scripts (Back-Up)\New-CFG\Hyper-V_enk.ps1'
        ...
        '1' 
        {
            ...
            switch ($isovm) {
                'A' 
                {
                    ...
                    Get-VM | Select-Object ComputerName,Name,State,VMNetworkAdapter,SwitchName,Uptime
                ...
                'B' 
                {
                    ...
                    Get-VM | Select-Object ComputerName,Name,State,VMNetworkAdapter,SwitchName
                    Show_MenuH # Herroep Hoofdmenu
                }
            }
        }
        ...
        {                    
            $CSVLocation = 'C:\Temp\GuestInfo.CSV'
                ...
                if ($foundVM.Name -eq $chosenVM) {
                    ...
                    # Verwijder HDD
                    $vhdPath = "C:\ProgramData\Microsoft\Windows\Hyper-V\Virtual Hard Disks\$($foundVM.Name).vhdx"
                    if ($akk -eq 'y') {Remove-Item -Path $vhdPath -Recurse -Force -Confirm:$True }
                    if ($akk -eq 'y') {Remove-VM -Name $foundVM.Name -Force ; Write-Host "Verwijdering succes!"
