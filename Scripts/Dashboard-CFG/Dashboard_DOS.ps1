...
function Show_MenuH{ # Hoofdmenu
    ...
    Write-Host "3: Druk op '3' voor Project-instellingen" # wijs naam toe van verzoeker/eindgebruiker 
...
function Show_Menu3{ # menu project
    Write-Host "=================================================="
    Write-Host "A: Druk op 'A' om Notes toe te voegen”
...
function Show_Menu3A{ # menu notes
    Write-Host "=================================================="
...
function Show_Menu3C{ # hoofdmenu groep
    Write-Host "=================================================="
            ...
            $project = Read-host "Kies optie"
                switch($project){
                    'A'
                    {
                        ...
                        Switch($notes){
                            'A' {
                        $notes = Read-Host 'Maak keuze' # of voeg CreationTime toe aan get-vm
                        $CSVLocation = 'C:\Temp\GuestInfo.CSV'
                        ...
                        Show_MenuH
                            ...
                            'B'
                            {
                                Read-Host "Voer uw notitie in"
                                $CSVLocGR2 = 'C:\Temp\Group.csv'
                                #NOTES VMGROUP, ga aan de slag met csv's appenden. Voeg een nieuwe kolom toe aan de tabel, zodat je notities kan toewijzen aan groepen.
                                $csv = Import-Csv $CSVLocGR2

                                foreach ($row in $CSVlocGR2)
                                {$row | Add-Member -NotePropertyName 'NotesGroup' -NotePropertyValue 'NotesGr'}

                                $csv | Export-Csv $CSVlocGR2 -NoTypeInformation
                            }
                            }
                            }
                    'B'
                    {
                        Write-Host "Optie B gekozen: Start/stop VM schedule"
                            # Roep het externe script aan
                            $scriptPath_enk = "C:\Users\Administrator\Desktop\Scripts\Dashboard-CFG\Hyper-V_enk.ps1"
                            $scriptPath = "C:\Users\Administrator\Desktop\Scripts\Dashboard-CFG\StartStopVMs.ps1"
                            & $scriptPath_enk
                            & $scriptPath
                    }
