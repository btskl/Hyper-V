                ...
                switch($project){
                    'A'
                    {
                        ...
                        switch ($menuKeuze) {
                                        ...
                                        # Zorg dat CSV geladen wordt
                                        $csv = Import-Csv $CSVLocGR2

                                        # Voeg de kolom toe als die nog niet bestaat
                                        if (-not ($csv | Get-Member -Name "NotesGroup" -MemberType NoteProperty)) {
                                            foreach ($row in $csv) {
                                                $row | Add-Member -NotePropertyName 'NotesGroup' -NotePropertyValue 'NotesGroup'
                                            }
                                        }
                                        ...
                                        # CSV weer exporteren
                                        $csv | Export-Csv $CSVLocGR2 -NoTypeInformation -Encoding UTF8
                                        Write-Host "Notitie '$notes' toegevoegd aan groep '$chosenGroup'"
                                        # toon-CSV in tabelformaat
                                        Import-Csv $CSVLocGR2 | Format-Table -AutoSize
                                        }}}
                                ...
                                'A'
                                    {
                                        ...
                                        Import-Csv 'C:\Temp\Group.CSV' | Format-Table -AutoSize
