...
foreach ($group in ($projects.GroupName | Sort-Object -Unique)) {
    ...
    foreach ($entry in $vmsInGroup) {
        ...
        . C:\Users\Administrator.TEST\Desktop\Scripts\Dashboard\Main\New_VM.ps1
        switch ($ISO) {
    ...
    # # === Scheduled Task maken voor projecten met vm's bij elkaar===
    # if ($endDate) {
    #     try {
...
# === Scheduled Task maken Voor VM's ===
    if ($endDate) {
        $desktopPath = [Environment]::GetFolderPath("Desktop")
        $notifScript = "$desktopPath\VM_End_${VMName}.txt"

        $action = New-ScheduledTaskAction -Execute "powershell.exe" `
            -Argument "-NoProfile -ExecutionPolicy Bypass -Command `"Add-Content -Path '$notifScript' -Value 'Einddatum bereikt voor VM $VMName ($group) op $endDate'`""

        $trigger = New-ScheduledTaskTrigger -Once -At (Get-Date $endDate)
        $principal = New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType ServiceAccount -RunLevel Highest
        $taskName = "VMEnd_$VMName"

        try {
            Register-ScheduledTask -Action $action -Trigger $trigger -TaskName $taskName -Principal $principal -Force | Out-Null
            Write-Host "📅 Taak '$taskName' ingepland voor $VMName ($endDate)"
        }
        catch {
            Write-Host "❌ Kon taak voor VM '$VMName' niet inplannen: $_"
        }
    }

Write-Host "`n✅ Alle projecten uit CSV uitgerold!" -ForegroundColor Green

# ssh-key nogsteeds een probleem. kijk daar thuis naar op jouw laptop, grote kans dat ie het daar niet doet. of maak nieuwe VM's aan om de procedure te testen.