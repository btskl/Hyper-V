...
# Controleer of de CSV al bestaat, anders aanmaken
if (-not (Test-Path $CSVlocGRnotes)) {
    # CSV aanmaken met juiste kolommen
...
# Bestaat de groep al in de CSV?
$row = $csv | Where-Object { $_.Name -eq $newgroup }

if ($row) {
    # Update bestaande entry
    $row.EndDate = if ($endDateParsed) { $endDateParsed.ToString("yyyy-MM-dd HH:mm") } else { $row.EndDate }
    if (-not $row.NotesGroup) { $row.NotesGroup = "" }
}
else {
...
# voeg lid toe
& C:\Users\Administrator.TEST\Desktop\Scripts\Dashboard\Create_Project\VirtualM.ps1

# notities
& C:\Users\Administrator.TEST\Desktop\Scripts\Dashboard\Create_Project\Notes.ps1

# laat overzicht zien
& C:\Users\Administrator.TEST\Desktop\Scripts\Dashboard\Create_Project\Overzicht.ps1
