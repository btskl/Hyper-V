...
function ws_2019{
    ...
    # geef hier aan of het bestand succesvol is overgezet of niet..
        if (Test-Path $diskPathws2019) {Add-VMDvdDrive $VMName -Path $diskPathws2019 ; Write-Host "`n✅ ISO-bestand is succesvol overgezet naar $diskPathws2019" -ForegroundColor Green} 
            else {scp -i "${env:USERPROFILE}\.ssh\id_rsa" "Administrator@${hostnameRDP}:\Users\Administrator\Desktop\VMs\WS2019.iso" "$diskPathws2019" ; Write-Warning "`n❌ 1ste overdracht mislukt. Bestand niet gevonden op $diskPathws2019, zet ISO bestand over!"}
...
function W10{
    ...
    # geef hier aan of het bestand succesvol is overgezet of niet..
        if (Test-Path $diskPathw10) {Add-VMDvdDrive $VMName -Path $diskPathw10 ; Write-Host "`n✅ ISO-bestand is succesvol overgezet naar $diskPathw10" -ForegroundColor Green} 
            else {scp -i "${env:USERPROFILE}\.ssh\id_rsa" "Administrator@${hostnameRDP}:\Users\Administrator\Desktop\VMs\Windows_10_LTSC.iso" "$diskPathw10" ; Write-Warning "`n❌ 1ste overdracht mislukt. Bestand niet gevonden op $diskPathw10, zet ISO bestand over!"}
