...
function ws_2019{
    ...
    function New-UniqueVHDPath {
    param([string]$VMName)

    $timestamp = (Get-Date).ToString("yyyyMMdd_HHmmss")
    return "C:\ProgramData\Microsoft\Windows\Hyper-V\Virtual Hard Disks\$VMName-$timestamp.vhdx"
    }

    # maak nieuwe vhd aan
    $vhdPath = New-UniqueVHDPath -VMName $VMName

    $VHDiskBytes = $DiskGB * 1GB

    New-VHD -Path $vhdPath -SizeBytes $VHDiskBytes -Dynamic | Out-Null
