...
function ws_2019{
    ...
    # stuur ISO-Bestand door naar variabele, d.m.v. public key.
    ...
    # geef hier aan of het bestand succesvol is overgezet of niet..
        ...
        if (Test-Path $diskPathws2019) {Add-VMDvdDrive $VMName -Path $diskPathws2019 ; Write-Host "`n✅ ISO-bestand is succesvol overgezet naar $diskPathws2019" -ForegroundColor Green} 
            else {scp -i "${env:USERPROFILE}\.ssh\keypair" "localabb@${hostnameRDP}:\Users\localabb\Desktop\VMs\WS2019.iso" "$diskPathws2019" 
            ; Write-Warning "`n❌ Overdracht mislukt. Bestand niet gevonden op $diskPathws2019, zet ISO bestand over!"}
